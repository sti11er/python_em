...
class OnlineEM_local_E:
    def __init__(self, num_topics=20, L=1, vocab_size=2000, 
                 alpha=0.1, beta=0.7, gamma=0.3, delta=0.9,
                 random_state=None):
    ...
    def compute_theta_d(self, p_tw):
        s1 = np.sum(p_tw, axis=1) # [n_topics, ]
        s2 = np.sum(p_tw)
        return s1 / (s2+self.eps)


    def local_e_step(self, doc, doc_idx):
        old_theta = self.theta[:, doc_idx].copy()

        term_indices = list(doc.keys())
        n_d = len(term_indices)
        
        theta_forward = np.zeros((self.num_topics, n_d))
        theta_backward = np.zeros((self.num_topics, n_d))
        
        n_t_expanded = np.tile(self.n_t[np.newaxis, :], (n_d, 1))

        p_raw = self.phi[term_indices, :] * n_t_expanded 
        p_ti = self.get_prob_matrix_by_counters(p_raw).T

        gamma_forward = np.full(n_d, 1)
        gamma_backward = np.full(n_d, 1)

        for _ in range(self.L):
            theta_forward[:, 0] = (gamma_forward[0]-self.gamma) * p_ti[:, 0]
            for i in range(1, n_d):
                theta_forward[:, i] = gamma_forward[i] * p_ti[:, i] + (gamma_forward[i]-self.gamma) * theta_forward[:, i-1]

            theta_backward[:, -1] = (gamma_backward[-1]-self.gamma) * p_ti[:, -1]
            for i in range(n_d-2, -1, -1):
                theta_backward[:, i] = gamma_backward[i] * p_ti[:, i] + (gamma_backward[i]-self.gamma) * theta_backward[:, i+1]

            theta = self.beta*theta_forward + (1-self.beta)*theta_backward
            p_raw = theta * (p_ti / (self.n_t[:, np.newaxis] + self.eps))
            p_ti = self.get_prob_matrix_by_counters(p_raw)

        self.n_wt_[term_indices, :] += p_ti.T
        self.n_t += p_ti.sum(axis=1)
        self.theta[:, doc_idx] = self.compute_theta_d(p_ti)
    ...
    def m_step(self):
        ...
        self.n_wt = self.delta * np.maximum(self.n_wt, self.eps) + self.alpha * np.maximum(self.n_wt_, self.eps)
    ...
    def compute_perplexity(self, doc_term_dict):
        """
        Вычисляет перплексию для набора документов
        
        Args:
            doc_term_dict: словарь {doc_id: {word: count}}
            
        Returns:
            perplexity: значение перплексии
        """

        if self.term_to_idx is None:
            raise ValueError("Vocabulary not initialized. Call init_vocabulary() first.")

        docs = doc_term_dict.values()
        n_dw = np.zeros((self.vocab_size, len(docs)))

        for doc_idx, doc in enumerate(docs):
            doc_indices = {self.term_to_idx[w]: cnt for w, cnt in doc.items() 
        ...
        p_wd = np.log(self.phi @ self.theta + self.eps)
        s1 = np.sum(n_dw * p_wd)
...
def main(random_state=42):
    ...
    # model.add_regularizer(SparseRegularizer(tau=0.1, target="phi"))
    # model.add_regularizer(DecorrelatorRegularizer(tau=0.05, target="phi"))
    # model.add_regularizer(SparseRegularizer(tau=0.05, target="theta"))
